...
from model.detect import _corroborate_network, score_reading, load_model

def test_graduated_confidence_marginal_vs_extreme():
    print('Testing graduated confidence marginal vs extreme...')
    # 1. Frozen Value
    ...
    print(f'  Frozen: streak=4 -> {conf_frozen_marginal}, streak=6 -> {conf_frozen_mid}, streak=10 -> {conf_frozen_extreme}')
    assert conf_frozen_marginal == 80.0
    assert 80.0 < conf_frozen_mid < 95.0
    assert conf_frozen_extreme == 95.0
    ...
    print(f'  Drift: accum=7.1 -> {conf_drift_marginal}, accum=10.5 -> {conf_drift_mid}, accum=14.5 -> {conf_drift_extreme}')
    assert 85.0 <= conf_drift_marginal < 86.0
    assert conf_drift_mid == 90.0
    assert conf_drift_extreme == 95.0
    ...
    print(f'  Fail-low: val=-8.0 streak=2 -> {conf_faillow_marginal}, val=-35.0 streak=6 -> {conf_faillow_extreme}')
    assert conf_faillow_marginal == 92.0
    assert conf_faillow_extreme == 98.0
    ...
    print(f'  Multivariate: single(3.1) -> {conf_mv_single_marginal}, single(6.0) -> {conf_mv_single_high}, conf(3.1) -> {conf_mv_conf_marginal}, conf(6.0) -> {conf_mv_conf_high}')
    assert conf_mv_single_high < conf_mv_conf_marginal
    ...
    print(f'  Spike: clean -> {conf_spike_clean}, partial -> {conf_spike_partial}')
    assert conf_spike_clean > conf_spike_partial

def test_spike_and_multivariate_untouched_by_network_corroboration():
    print('Testing spike and multivariate isolation from network corroboration...')
    ...
    assert res_spike_empty['confidence_bonus'] == 0.0
    assert res_spike_swing['confidence_bonus'] == 0.0
    assert res_spike_swing['relabel_fault_type'] is None
    ...
    assert res_mv_empty['confidence_bonus'] == 0.0
    assert res_mv_swing['confidence_bonus'] == 0.0
    assert res_mv_swing['relabel_fault_type'] is None
    print('  Spike and multivariate remain completely unaffected by peer corroboration.')

def test_is_anomaly_never_flipped_by_network_corroboration():
    print('Testing is_anomaly never flipped by corroboration...')
    # Use 2-point history so rate-of-change (temp_roc_1h) can be computed
    ...
    assert 'is_anomaly' not in res_drift or res_drift.get('is_anomaly') is not False
    print(f'  Drift with regional peers -> state={res_drift["state"]}, bonus={res_drift["confidence_bonus"]}, relabel={res_drift["relabel_fault_type"]}')
    assert res_drift['confidence_bonus'] == 3.0
    assert res_drift['relabel_fault_type'] == 'REGIONAL_EVENT'

if __name__ == '__main__':
    test_graduated_confidence_marginal_vs_extreme()
    test_spike_and_multivariate_untouched_by_network_corroboration()
    test_is_anomaly_never_flipped_by_network_corroboration()
    print('ALL DEMONSTRATION & VERIFICATION TESTS PASSED!')
